...
const PromptCard = ({post, handleTagClick, handleEdit, handleDelte}) => {
  ...
  return (
    <div className="prompt_card">
      <div className="flex justify-between items-start gap-5">
        <div className="flex-1 flex justify-start items-center gap-3 cursor-pointer">
          ...
          <div className="flex flex-col">
             <h3 className="font-satoshi font-semibold text-gray-500">{post.creator.name}</h3>
      ...
      {session?.user.id === post.creator._id && pathName ==="/profile" && (
        <div className="mt-5 flex-center gp-4 border-t border-gray-100 pt-3">
          <p className="font-inter text-sm green_gradient cursor-pointer" onClick={handleEdit}>Edit</p>
          <p className="font-inter text-sm orange_gradient cursor-pointer" onClick={handleDelte}>Delete</p>
        </div>
      )}
    </div>
  )
